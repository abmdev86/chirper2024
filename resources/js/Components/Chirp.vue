...
<template>
    <div class="p-6 flex space-x-2">
        ...
        <div class="flex-1">
            ...
            <!-- <p class="nt-4 text-lg text-gray-900">
                {{ chirp.message }}
            </p> -->
            <form
            v-if="editing" @submit.prevent="form.put(route('chirps.update', chirp.id), {onSuccess:()=> editing = false})"
            ...
            <p
            v-else class="mt-4 text-lg text-gray-900"
            >{{ chirp.message }}</p>
        </div>
    </div>
</template>
